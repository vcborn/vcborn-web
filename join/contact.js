/*
 * 最初に
 *  このスクリプトはほとんどは私ではない誰かが書きました。
 *  そのうえで、私Sorakimeが修正を加えて、動作するようにしました。
 *  書いてくれた方には本当に感謝御礼申し上げます。
 * 変更・追加点
 *   コメントの追加
 * 	 TwitterのIDの確認の記述
 *   部署の確認の記述
 *   HTMLの<button>を<a>に変更
 * 追記
 *   エラーメッセージを追加しました
 */
const validate = function () {
  ...
  // TwitterのID確認
  if (
    !document.form.twitterid.value ||
    document.form.twitterid.value == "@"
  ) {
    swal({
      title: 'エラー',
      text: 'TwitterのIDを入力してください。',
      icon: 'error',
      button: 'OK'
    });
    errorElement(document.form.twitterid, "TwitterのIDを入力してください");
    flag = false;
  } else if (document.form.twitterid.value.indexOf("@") == -1) {
    document.form.twitterid.value = `@${document.form.twitterid.value}`;
  }

  if (!document.form.group.value) {
    swal({
      title: 'エラー',
      text: '所属中のグループ（なければなし）を入力してください。',
      icon: 'error',
      button: 'OK'
    });
    errorElement(document.form.group, "所属中のグループ（なければなし）を入力してください")
    flag = false;
  }

  if (!document.form.active.value) {
    swal({
      title: 'エラー',
      text: '平均浮上日数を選択してください。',
      icon: 'error',
      button: 'OK'
    });
    errorElement(document.form.active, "平均浮上日数を選択してください")
    flag = false;
  }

  // 部署の確認
  let checkFlag = false;
  // すべてのチェックボックスの個数を取得してforEachする
  document.querySelectorAll(".checks input").forEach((i, count) => {
    if (document.querySelectorAll(".checks input")[count].checked) {
      checkFlag = true;
    }
  });
  ...
  if (!checkFlag) {
    swal({
      title: 'エラー',
      text: 'いずれか一つを選択してください。',
      icon: 'error',
      button: 'OK'
    });
    errorElement(
      document.getElementsByClassName("checks")[0],
      "いずれか一つを選択してください"
    );
  }
  ...
  if (!document.form.content.value) {
    ...
    errorElement(document.form.content, "文章を入力してください");
    flag = false;
  }
